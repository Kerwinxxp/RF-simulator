function [pdArray, isWithinBldg1, datalats, datalons] = coverage_test(txs, varargin)
...
disp("Validating the parameters")
tic
% Validate parameters
...
if pm.isMultipathModel && strcmp(type,"efield")
    error(message("shared_channel:rfprop:InvalidRayTracingCoverageType"))
end
...
disp("Generating Location Grid containing data range from transmitter site")
tic
...
disp("Computing Building Coordinates")
tic
datalats1 = datalats;
datalons1 = datalons;
isWithinBldg1 = isWithinAnyBuilding(map.BuildingsArray, datalats, datalons);
% Trim grid locations to those which are not inside buildings
if pm.isMultipathModel && isa(map,'siteviewer')
    isWithinBldg = isWithinAnyBuilding(map.BuildingsArray, datalats, datalons);
    datalats = datalats(~isWithinBldg);
    datalons = datalons(~isWithinBldg);
end
toc
...
if isempty(datalats)
    ...
    if outputRequested
        varargout = {propagationData.empty};
    else
...
disp("Computing the Signal Strength for each receiver site")
disp("from this place")
numTx = numel(txs);
pdArray = cell(1, numTx);   % To store power distribution for each transmitter
for i = 1 : numTx
    disp("------------")
    disp(i)
    disp("------------")
    ...
    try
        if strcmp(rxLocationsLayout,'grid')
            ...
            [datalats, datalons, ~, data] = radialReceiverLocationsLayoutData(txs(i), txsCoords, type, ...
    ...
    disp("Setting up Output variable")
    tic
