classdef trajectory
    ...
    properties
        ...
        End
        Geopoints
        TargetLatLon double = [NaN NaN];  % [lat lon] 固定目标
        Angles       double = [];         % 1×N，每个采样点的角度（度）
        AngleMode    char   = 'FixedTarget'; % 'FixedTarget' 或 'FixedTheta'
        ThetaDeg     double = NaN;        % 固定波束角（如果用 FixedTheta）
        BeamwidthDeg double = NaN;   % 每条轨迹固定束宽（单位：度）
    ...
    methods
        ...
        function lat_lon_table = syncCordinates(obj, tbl)
            lat_lon_table = zeros(100, 2);
            for i = 1:100
                lat = obj.Geopoints(1,i).Latitude;
                ...
                lon = obj.Geopoints(1,i).Longitude;
                for j = 1:(height(tbl) - 1)
